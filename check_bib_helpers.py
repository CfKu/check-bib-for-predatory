...
def print_colored_status(text, status_color_id=0):
    ...
    if status_color_id < len(COLORS):
        if status_color_id == 0:
            print(COLORS[status_color_id] + ">>> " + text + colorama.Style.RESET_ALL)
        else:
            print(COLORS[status_color_id] + ">>> " + colorama.Style.RESET_ALL + text)
    else:
        print(text)
...
def highlight_similarity(hl_words, sentence):
    re_from = (
        r"("
        + r"|".join(hl_words.replace(r"(", r"\(").replace(r")", r"\)").split(" "))
        + r")"
    )
